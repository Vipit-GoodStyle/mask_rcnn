...
class DataGenerator(object):
    ...
    def __generate_hmap(self, cvmat, kpAnnolst):
        ...
        for i, _kpAnn in enumerate(kpAnnolst):
            ...
            gthmp[ int(top_y):int(bottom_y), int(top_x):int(bottom_x), i] = gaussMask[int(top_y_offset):int(top_y_offset + bottom_y-top_y),
                                                                  int(top_x_offset):int(top_x_offset + bottom_x-top_x)]
